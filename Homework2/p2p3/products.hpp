...
enum FutureType {CME, BONDFUTURE};

class Future : public Product{
public:
  Future(string _productId, float _coupon, FutureType _futureType, float _strike, date _maturityDate);
  Future();
...
public:
  EuroDollarFuture(string _productId, float _strike, float _coupon, date _maturityDate, date _loanPayDate);
  EuroDollarFuture();

  date GetLoanPayDate() const;
...
public:
  BondFuture(string _productId, string _bondId, float _strike, float _coupon, date _maturityDate);
  BondFuture();

  string GetBondId() const;
...
Future::Future(string _productId, float _coupon, FutureType _futureType, float _strike, date _maturityDate):Product(_productId, FUTURE){
  strike = _strike;
  coupon = _coupon;
...
float Future::GetCoupon() const{return coupon;}

FutureType Future::GetFutureType() const{return futureType;}

date Future::GetMaturityDate() const{return maturityDate;}

float Future::GetStrike() const{return strike;}

// Overload the << operator to print out the bond
ostream& operator<<(ostream &output, const Future &future){
    switch(future.futureType){
        case BONDFUTURE:cout<<"Bond Future";break;
        case CME:cout<<"EuroDollar Future";break;
    }
  output << " " << future.coupon << " "  << future.strike << " " << future.GetMaturityDate();
  return output;
}

EuroDollarFuture::EuroDollarFuture(string _productId, float _strike, float _coupon, date _maturityDate, date _loanPayDate)
  : Future(_productId,_coupon,CME,_strike,_maturityDate){
    loanPayDate = _loanPayDate;
  }

EuroDollarFuture::EuroDollarFuture():Future(){}

date EuroDollarFuture::GetLoanPayDate() const {return loanPayDate;}

ostream& operator<<(ostream &output, const EuroDollarFuture &future){
  output << future.coupon << " "  << future.GetCoupon() << " "  << future.GetStrike() << " " << future.GetMaturityDate();
  return output;
}

BondFuture::BondFuture(string _productId, string _bondId, float _strike, float _coupon, date _maturityDate):
  Future(_productId,_coupon,BONDFUTURE,_strike,_maturityDate){
    bondId = _bondId;
}

BondFuture::BondFuture():Future(){}

string BondFuture::GetBondId() const{
  return bondId;
}

ostream& operator<<(ostream &output, const BondFuture &future){
  output << future.bondId << " " << future.GetCoupon() << " "  << future.GetStrike() << " " << future.GetMaturityDate();
  return output;
}
